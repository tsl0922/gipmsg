...
SendDlg *
SendDlgOpen(User *user, bool *is_new) {
	GList *entry;
	SendDlg *dlg;
						...
						gchar *groupName)
{
	...
	cpixbuf = gdk_pixbuf_new_from_file(ICON_PATH "expender-closed.png", NULL);
...
static GList *
users_tree_get_child_users(GtkTreeModel *model,
								GtkTreeIter *parent)
{
	GtkTreeIter iter;
	gint type;
	...
	if(gtk_tree_model_iter_children(model, &iter, parent)) {
		do {
			gtk_tree_model_get(model, &iter, TYPE_COLUMN, &type, -1);
			if(type == COL_TYPE_USER) {
				gtk_tree_model_get(model, &iter, DATA_COLUMN, &user, -1);
...
void
modify_status_icon(char *tip, char *iconfile, bool blinking)
{
	gtk_status_icon_set_tooltip_markup(main_win.trayIcon, tip);
	if(iconfile) gtk_status_icon_set_from_file(main_win.trayIcon, iconfile);
	gtk_status_icon_set_blinking(main_win.trayIcon, blinking);
}

void
notify_sendmsg(Message *msg)
{
	bool is_new;
