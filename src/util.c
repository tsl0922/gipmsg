...
AddrInfo *get_sys_addr_info(int *len_p)
{
	...
	if (GetIpAddrTable(pIPAddrTable, &dwSize, 0) == NO_ERROR) {
		...
		for (i = 0; i < sum; i++) {
			...
			retAddr[retLen].br_addr =
			    (pIPAddrTable->table[i].dwAddr & pIPAddrTable->
			     table[i].dwMask) | ~pIPAddrTable->table[i].dwMask;
			retLen++;
		}
	}
	if (pIPAddrTable) {
 ...
 */
char *seprate_token(const char *buf, char seprator, char **handle)
{
	char *sp, *ep;
...
ssize_t xsend(SOCKET sock, const void *buf, size_t count, int flags)
{
	...
	while (n_send != count && size != 0) {
		if ((size =
		     SEND(sock, (char *)buf + n_send, count - n_send,
			  flags)) == -1) {
			if (errno == EINTR)
				continue;
...
ssize_t xrecv(SOCKET sock, const void *buf, size_t count, int flags)
{
	...
	while (n_recv != count && size != 0) {
		if ((size =
		     RECV(sock, (char *)buf + n_recv, count - n_recv,
			  flags)) == -1) {
			if (errno == EINTR)
				continue;
...
ssize_t xread(int fd, const void *buf, size_t count)
{
	...
	while (n_read != count && size != 0) {
		if ((size =
		     READ(fd, (char *)buf + n_read, count - n_read)) == -1) {
			if (errno == EINTR)
				continue;
...
ssize_t xwrite(int fd, const void *buf, size_t count)
{
	...
	while (n_write != count && size != 0) {
		if ((size =
		     WRITE(fd, (char *)buf + n_write, count - n_write)) == -1) {
			if (errno == EINTR)
				continue;
