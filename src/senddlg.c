...
typedef void *(*LinkButtonFunc)(GtkLinkButton *button, const gchar *link_, gpointer data);
...
static void recv_text_add_button(SendDlg *dlg, const char *label, const char *tip_text, 
	LinkButtonFunc func, gpointer data);
static gboolean retry_message_handler(gpointer data);

SendDlg *
senddlg_new(User *user)
{
...
static void
on_retry_link_clicked(GtkLinkButton *button, const gchar *link_, gpointer data)
{
	struct ButtonCallbackArgs *args = (struct ButtonCallbackArgs *)data;
	send_msg((SendEntry *)(args->data));
	args->dlg->timerId = g_timeout_add(MSG_RETRY_INTERVAL, retry_message_handler, args->dlg);
	...
	while(list) {
		...
		if(entry->status != ST_DONE) {
			if(entry->retryCount++ < MSG_SEND_MAX_RETRY) {
				...
				snprintf(buf, MAX_BUF, "Failed to send the following message to %s:\n%s",
					entry->user->nickName, entry->msg);
				senddlg_add_info(dlg, buf);
				//create retry button
				entry->retryCount = 0;//reset retryCount
				struct ButtonCallbackArgs *args = (struct ButtonCallbackArgs *)malloc(sizeof(struct ButtonCallbackArgs));
				args->dlg = dlg;
				args->data = entry;
				recv_text_add_button(dlg, "Retry", "Resend message.", (LinkButtonFunc)on_retry_link_clicked, args);
			}
		}
		else {
			...
			dlg->send_list = g_list_delete_link(dlg->send_list, list);
...
bool
is_same_packet(SendDlg *dlg, Message *msg)
{
	if(dlg->msg == NULL) return false;
	
	return (dlg->msg->fromAddr == msg->fromAddr && dlg->msg->packetNo== msg->packetNo);
}

static void
make_packet(SendEntry *entry)
{
...
send_file_attach_info(SendDlg *dlg)
{
	...
	if(files) send_file_info(dlg->user->ipaddr, files);
	gtk_list_store_clear(GTK_LIST_STORE(model));
	snprintf(buf, MAX_BUF, "You request send %d files, %d folders(total: %d) to %s.",
		file_num, folder_num, file_num+folder_num, dlg->user->nickName);
	senddlg_add_info(dlg, buf);
}
...
static void
update_send_file_tree(GFile *file, SendDlg *dlg) {
	GtkTreeModel *model;
	GtkTreeIter iter;
	GtkIconInfo *iconinfo;
	GdkPixbuf *pixbuf;
	...
	if(gtk_tree_model_get_iter_first(model, &iter)) {
		gchar *path;
		do {
			gtk_tree_model_get(model, &iter, SF_COL_PATH, &path, -1);
	...
	gtk_list_store_set(GTK_LIST_STORE(model), &iter,
		...
		SF_COL_TYPE, (info->attr & IPMSG_FILE_DIR)?"folder":"file",
		SF_COL_SIZE, filesize,
		SF_COL_PATH, info->path,
		SF_COL_INFO, info,
		-1
		);
	g_object_unref(file);
...
senddlg_ui_init(SendDlg *dlg) {
	...
	pb = gdk_pixbuf_new_from_file_at_size(ICON_PATH "icon.png", 40 , 40 , NULL);
	...
	gtk_box_pack_start(GTK_BOX(dlg->rt_box) , dlg->rt_evbox , FALSE , FALSE , 0);
	dlg->photo_frame = gtk_frame_new(NULL);
	gtk_frame_set_shadow_type(GTK_FRAME(dlg->photo_frame) , GTK_SHADOW_ETCHED_IN);
	...
	gtk_box_pack_start(GTK_BOX(dlg->rt_box) , dlg->photo_frame , FALSE , FALSE , 0);
	...
	gtk_box_pack_start(GTK_BOX(dlg->rb_box) , dlg->rb_evbox , FALSE , FALSE , 0);
	dlg->send_file_scroll = gtk_scrolled_window_new(NULL, NULL);
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(dlg->send_file_scroll),
									GTK_POLICY_AUTOMATIC,
									GTK_POLICY_AUTOMATIC);
	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(dlg->send_file_scroll),
									GTK_SHADOW_ETCHED_IN);
	gtk_widget_set_size_request(dlg->send_file_scroll, 180, 120);
	dlg->send_file_tree = gtk_tree_view_new_with_model(create_file_tree_model());
	create_file_tree_column(GTK_TREE_VIEW(dlg->send_file_tree));
	gtk_container_add(GTK_CONTAINER(dlg->send_file_scroll), dlg->send_file_tree);
	gtk_box_pack_start(GTK_BOX(dlg->rb_box) , dlg->send_file_scroll , FALSE , FALSE , 0);
	gtk_box_pack_start(GTK_BOX(rvbox) , dlg->rb_box , TRUE , TRUE , 0);
	
	dlg->recv_file_scroll = gtk_scrolled_window_new(NULL, NULL);
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(dlg->recv_file_scroll),
									GTK_POLICY_AUTOMATIC,
									GTK_POLICY_AUTOMATIC);
	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(dlg->recv_file_scroll),
									GTK_SHADOW_ETCHED_IN);
	gtk_widget_set_size_request(dlg->recv_file_scroll , 180 , 160);
	dlg->recv_file_tree = gtk_tree_view_new_with_model(create_file_tree_model());
	create_file_tree_column(GTK_TREE_VIEW(dlg->recv_file_tree));
	gtk_container_add(GTK_CONTAINER(dlg->recv_file_scroll) , dlg->recv_file_tree);
...
void
senddlg_add_message(SendDlg *dlg, const char *msg, bool issendmsg)
{
	...
	gtk_text_buffer_insert_with_tags_by_name(dlg->recv_buffer
					, &dlg->recv_iter , text , -1 , color , NULL);
...
void 
senddlg_add_info(SendDlg *dlg, const char *msg)
{
	GtkTextChildAnchor *anchor;
	GtkWidget          *image;
	gchar               text[MAX_BUF];
	...
	anchor = gtk_text_buffer_create_child_anchor(dlg->recv_buffer , &dlg->recv_iter);
	gtk_text_view_add_child_at_anchor(GTK_TEXT_VIEW(dlg->recv_text), image , anchor);

	g_snprintf(text, MAX_BUF, "  (%s) %s\n", time, msg);
	gtk_text_buffer_insert_with_tags_by_name(dlg->recv_buffer, &dlg->recv_iter,
					text , -1 , "blue", "small", "tm10", NULL);
	gtk_text_buffer_insert(dlg->recv_buffer , &dlg->recv_iter , "\n" , -1);
	...
	LinkButtonFunc func, gpointer data)
{
	...
	anchor = gtk_text_buffer_create_child_anchor(dlg->recv_buffer, &dlg->recv_iter);
	gtk_text_view_add_child_at_anchor(GTK_TEXT_VIEW(dlg->recv_text),widget, anchor);
...
create_file_tree_column(GtkTreeView *file_tree) {
	...
	column = gtk_tree_view_column_new_with_attributes("Name",
		renderer, "pixbuf", SF_COL_ICON, NULL);
	renderer = gtk_cell_renderer_text_new();
	gtk_tree_view_column_pack_start(column, renderer, FALSE);
	gtk_tree_view_column_set_attributes(column, renderer, "text", SF_COL_NAME, NULL);
	gtk_tree_view_append_column(file_tree, column);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("Type",
		renderer, "text", SF_COL_TYPE, NULL);
	gtk_tree_view_append_column(file_tree, column);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("Size",
		renderer, "text", SF_COL_SIZE, NULL);
	gtk_tree_view_append_column(file_tree, column);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("Path",
		renderer, "text", SF_COL_PATH, NULL);
	gtk_tree_view_append_column(file_tree, column);
}
...
update_dlg_info(SendDlg *dlg) {
	...
	title = g_strdup_printf("Chating with %s(IP: %s)", dlg->user->nickName, addr_str);
	name = g_strdup_printf("<b>%s</b>", dlg->user->nickName);
	info = g_strdup_printf("<i>%s</i>(<i>%s</i>) IP: %s Group: <i>%s</i>",
		dlg->user->userName, dlg->user->hostName, addr_str, dlg->user->groupName);
	FREE_WITH_CHECK(addr_str);

	if(dlg->user->headIcon) {
		pixbuf = gdk_pixbuf_new_from_file_at_size(dlg->user->headIcon, 30, 30, NULL);
	...
	if(dlg->user->photoImage) {
		pixbuf = gdk_pixbuf_new_from_file_at_size(dlg->user->photoImage, 140, 140, NULL);
...
bool
notify_send_finish(SendDlg *dlg, ulong ipaddr, packet_no_t packet_no) 
{
	GList *list = dlg->send_list;
	bool ret = false;
...
static void
update_recv_file_tree(SendDlg *dlg, GSList *files)
{
	GtkTreeModel *model;
	GdkPixbuf *pixbuf;
	FileInfo *info;
	...
	while(entry) {
		...
		format_filesize(filesize, info->size);
		if(is_folder)
			pixbuf = gdk_pixbuf_new_from_file(ICON_PATH "folder.png", NULL);
		else
			pixbuf = gdk_pixbuf_new_from_file(ICON_PATH "file.png", NULL);
		gtk_list_store_append(GTK_LIST_STORE(model), &iter);
		gtk_list_store_set(GTK_LIST_STORE(model), &iter,
			RF_COL_ICON, pixbuf,
...
static void
show_recv_file_area(SendDlg *dlg)
{
	if(dlg->is_recv_file) return;
...
void
senddlg_add_fileattach(SendDlg *dlg, GSList *files)
{
	char buf[MAX_BUF];
